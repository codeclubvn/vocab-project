    ...
    return (
        <div className={`w-full cursor-pointer flex-1 flex flex-col md:flex-row ${props?.className}`}>
            ...
            <div className="flex-1">
                ...
                { props.author && (<p>{props?.numberOf} Người học | Tạo bởi {props?.author}</p>)}
                {(props.totalWords && props.wordLearnt)  && (<p>{`${(props?.wordLearnt / props.totalWords).toFixed(1)}%`} {`${props?.wordLearnt}/${props?.totalWords} Từ đang học`} </p>)}
                {props?.description && <Text className='line-clamp-2' as='p' size='2'>{props?.description}</Text>}
            </div>
        </div>
    )
}